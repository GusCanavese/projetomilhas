...
def home(request):
    ...
    certificado = request.POST.get('certificado', None)
    
    
    # pega os valores das variáveis e cadastra no banco de dados
    if request.method == "POST":
        Membro.objects.create(nome=nome, data=data, curso=curso, matricula=matricula, email=email, entrada=entrada, saida=saida, pontos=pontos, cargo=cargo, info=info, foto=foto, certificado=certificado)
